/**
 * SUATU SAAT v2 — Screen 3: Daftar Isi / TOC (Fullscreen WebApp)
 */
...
export class TocScreen {
  ...
  constructor(container: HTMLElement) {
    ...
    const chaptersHTML = CHAPTERS.map((ch, idx) => {
      ...
      return `
        <div class="toc-bab-block ${isOpen}" style="border-bottom: 1px solid var(--line); padding: 16px 0;">
          <div class="toc-bab-head" style="display: flex; justify-content: space-between; align-items: flex-start; cursor: pointer;">
            ...
            <div class="toc-chev" style="font-size: 14px; color: var(--bone-dim); margin-top: 4px;">▼</div>
          </div>
          <div class="toc-subitems" style="display: flex; flex-direction: column; gap: 10px; margin-top: 14px;">
            ${pagesHTML}
          </div>
        </div>
      `;
    ...
    this.el.innerHTML = `
      <!-- Top Header -->
      <div class="ph-header">
        <span style="font-family: var(--sans); font-size: 16px; font-weight: 600; letter-spacing: 1.5px; color: var(--bone);">DAFTAR ISI</span>
        <span class="close-btn" id="toc-close-btn">✕</span>
      </div>

      <!-- TOC Scroll Area -->
      <div class="toc-scroll" style="flex: 1; overflow-y: auto; padding: 0 24px 24px;">
        ${chaptersHTML}

        <!-- Penutup Row -->
        <div class="toc-bab-block" style="border-bottom: 1px solid var(--line); padding: 16px 0; display: flex; justify-content: space-between; align-items: center; cursor: pointer;" id="toc-penutup-row">
          <div>
            <div class="toc-bab-eyebrow">PENUTUP</div>
            <div class="toc-bab-name" style="font-size: 18px;">Aku Telah Pulang: Titik Nol Keheningan</div>
          </div>
          <span style="font-size: 13px; color: var(--bone-faint);">74</span>
        </div>

        <div style="text-align: center; font-size: 11px; color: var(--bone-faint); padding: 20px 0 10px;">
          Total 74 Halaman
        </div>
